...
class ContextMenuSpawner:
    ...
    def make_context_menu(self, evt):
        ...
        self.click_tracker = TrackOutsideClick(self.parents[0])
...
class EditableLabel(tk.Frame):
    ...
    def edit(self):
        ...
        self.click_tracker.bind(self.save)

class TrackOutsideClick:
    """
    Register when click events occur outside of a given widget
    """
    def __init__(self, domain: tk.Widget):
        self.funcid1: str
        self.funcid2: str
        self.funcid3: str
        self.funcid4: str

        self.domain: tk.Widget = domain
        self.bound: bool = False

    def bind(self, callback: Callable[[], Optional[str]]):
        ...
        wrapped_callback = lambda e: (callback() if e.widget != self.domain else None)

        self.funcid1 = self.domain.winfo_toplevel().bind("<1>", wrapped_callback, "+")
        self.funcid2 = self.domain.winfo_toplevel().bind("<2>", wrapped_callback, "+")
        self.funcid3 = self.domain.winfo_toplevel().bind("<3>", wrapped_callback, "+")
        self.funcid4 = self.domain.winfo_toplevel().bind("<Configure>", lambda e: callback if e.widget != self.domain else None, "+")

    def unbind(self):
        self.domain.winfo_toplevel().unbind("<1>", self.funcid1)
        self.domain.winfo_toplevel().unbind("<2>", self.funcid2)
        self.domain.winfo_toplevel().unbind("<3>", self.funcid3)
        self.domain.winfo_toplevel().unbind("<Configure>", self.funcid4)
